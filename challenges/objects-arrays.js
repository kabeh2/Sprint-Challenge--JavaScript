...
/////////////////////////////////////
////// Use for loop with an if statement that checks university presence then pushes them to array
...
/////////////////////////////////////
////// Use for loop with an if statement that checks email and name presence then pushes them to array
const contactInfo = [];

for (let i = 0; i < graduates.length; i++) {
  if (graduates[i].email && graduates[i].first_name) {
...
/////////////////////////////////////
////// Use .includes() to find the string
...
const lowerCase = [];
zooAnimals.map(el => lowerCase.push(el.animal_name.toString().toLowerCase()));
...
zooAnimals
  .filter(el => el.population < 5)
  .map(el =>
    largerPopulation.push(
      ` Name: ${el.animal_name}, Population: ${el.population}`
    )
  );
